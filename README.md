...
Note that the [original implementation](https://github.com/wozhouh/my-deep-retrieval/blob/master/test.py) provides an option of multi-resolution, and we find that it does improve the performance on Oxford (measured by mAP). It is a natural idea whether the feature maps of different sizes extracted from the middle layers could improve the descriptor and take similar effects, so we have the following attempts.
