...
class CoverData:
    ...
    def cal_precision(self, sim):
        ...
        for q in range(self.num_queries):
            ...
            top_answer = list(idx[q, : top_k])
            cnt_correct = len([i for i in top_answer if i in self.a_idx[q]])
    ...
    def cal_mAP(self, sim):
        ...
        for q in range(self.num_queries):
            ...
            for d in range(self.num_dataset):
                top_answer = list(idx[q, : d])
                cnt_correct = len([i for i in top_answer if i in q_ans])
                assert cnt_correct >= cnt_correct_last
                assert cnt_correct <= len(q_ans)
                if cnt_correct > cnt_correct_last:
                    recall[cnt_correct-1] = float(cnt_correct) / float(d+1)  # precision under the given recall
            ...
            print("AP of cls %d: %f" % (q, q_AP[q]))  # print for checking as the function is too slow ...
...
if __name__ == '__main__':
    ...
    print('precision: %f' % cData.cal_precision(sim))
