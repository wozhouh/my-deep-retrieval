...
class RigidGridLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        assert bottom[0].data.shape[1] == 3, 'The input should be a 3-channel RGB color image in 1x3xHxW format'

    def reshape(self, bottom, top):
        tmp_shape = [self.num_region, 5]
        top[0].reshape(*tmp_shape)

    def forward(self, bottom, top):
        H = bottom[0].data.shape[2]
        W = bottom[0].data.shape[3]
        all_regions = [self.get_rmac_region_coordinates(H, W, 2)]
        R = self.pack_regions_for_network(all_regions)
        assert R.shape[0] >= 8, 'The number of regions should be no less than 8'
        top[0].data[:] = R[0: self.num_region, :]
        # top[0].data[:] = R
